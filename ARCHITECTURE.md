...
The ecosystem outputs real-time metrics summarizing its current growth state to the `README.md` dashboard:
- **Epoch (Days Alive)**: The total age of the ecosystem. This physically increments by 1 every time the Morning Sprout action is triggered.
- **Trees Planted**: The number of base trunks currently growing. When one tree reaches max maturity, the algorithm drops a new seed to create a forest over time.
- **Total Branches**: The absolute count of line geometries rendered in the SVG, representing every single stochastic growth decision the algorithm has made since Epoch 0.
- **Total Foliage**: The absolute count of colored circles (leaves and blossoms) generated by the Midday Bloom logic.

## 6. Automation Strategy (The "2-3 Daily Commits" Guarantee)
